...
REAL*8, ALLOCATABLE, DIMENSION(:,:,:) :: br_face, bz_face
...
ALLOCATE(br_face(-2:nx_2+3,-2:ny_2+3,-2:nz_2+3))
...
DO l = 1, nz_2
  DO k = 1, ny_2
    DO j = 1, nx_2
      br_face(j,k,l) = -(A_corner(j,k,l) - A_corner(j,k,l-1))/dz2(l)
...
DO l = 1, nz_2
  DO k = 1, ny_2
    DO j = 1, nx_2
      prim2(ibx,j,k,l) = 0.5D0*(br_face(j,k,l) + br_face(j-1,k,l))
...
DO l = 1, nz_2
  DO k = 1, ny_2
    DO j = 1, nx_2
      div_b = (xF2(j)*br_face(j,k,l) - xF2(j-1)*br_face(j-1,k,l))/(x2(j)*dx2(j)) + & 
...
B_0 =  SQRT(2.0d0*(k_poly*rho_fac**(gamma))/p_beta)
! Get cell-centered magnetic fields by averaging !
DO l = 1, nz_2
  DO k = 1, ny_2
    DO j = 1, nx_2
      !prim2(ibx,j,k,l) = 0.0D0*(br_face(j,k,l) + br_face(j-1,k,l))
      !prim2(ibz,j,k,l) = 0.0D0*(bz_face(j,k,l) + bz_face(j,k,l-1))
      IF(x2(j) > s_0 .AND. x2(j) < s_1) THEN
        prim2(ibz,j,k,l) = B_0
      END IF
    END DO
  END DO
...
! orbital time scasle
t_scale = 1/(SQRT(h_square)*s_max**(-q_grad))

! Set output profile interval !
total_time = 100.0d0 !10.0d0*t_scale
output_profiletime = 10.0d0 !t_scale
