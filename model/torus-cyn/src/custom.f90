...
!$OMP PARALLEL DO COLLAPSE(3) SCHEDULE(STATIC) PRIVATE(radius, factor, diff, dphidr, dphidz) FIRSTPRIVATE(rho_min2)
!$ACC PARALLEL LOOP GANG WORKER VECTOR COLLAPSE(3) DEFAULT(PRESENT) PRIVATE(radius, factor, diff, dphidr, dphidz) FIRSTPRIVATE(rho_min2)
DO l = nz_min_2, nz_part_2
  DO k = ny_min_2, ny_part_2
    DO j = nx_min_2, nx_part_2
      ...
      radius = DSQRT(x2(j)*x2(j) + z2(l)*z2(l))
      dphidr = x2(j)/radius/((radius - r_sh)*(radius - r_sh))
      dphidz = z2(l)/radius/((radius - r_sh)*(radius - r_sh))
