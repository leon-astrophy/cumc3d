...
REAL*8, ALLOCATABLE, DIMENSION(:,:,:) :: br_face, by_face
  ...
  DO k = 1, ny_2
    DO j = 1, nx_2
      ...
      IF(prim2(irho2,j,k,l) >= rho_cut) THEN
        A_phi(j,k,l) = (prim2(irho2,j,k,l) - rho_cut)
      END IF
...
DO l = 1, nz_2
  DO k = 1, ny_2
    DO j = 1, nx_2
      br_face(j,k,l) = (SIN(yF2(k))*A_corner(j,k,l) - SIN(yF2(k-1))*A_corner(j,k-1,l))/dy2(k)/(xF2(j))/SIN(y2(k))
...
DO l = 1, nz_2
  DO k = 1, ny_2
    DO j = 1, nx_2
      div_b = (xF2(j)**2*br_face(j,k,l) - xF2(j-1)**2*br_face(j-1,k,l))/(x2(j)**2*dx2(j)) + & 
...
total_time = 10.0d0*t_scale
output_profiletime = t_scale
