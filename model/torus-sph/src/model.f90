  ...
  DO k = 1, ny_2
    DO j = 1, nx_2
      ...
      IF(rho >= rho_a .AND. s_eq >= s_0 .AND. s_eq <=s_1) THEN
        ...
        omega = DSQRT(h_square)*s_eq**(-q_grad)
...
DO l = 1, nz_2
  DO k = 1, ny_2
    DO j = 1, nx_2
      bx_face(j,k,l) = (DSIN(yF2(k))*A_corner(j,k,l) - DSIN(yF2(k-1))*A_corner(j,k-1,l))/(xF2(j))/dcos2(k)!dy2(k)/DSIN(y2(k))
...
DO l = 1, nz_2
  DO k = 1, ny_2
    DO j = 1, nx_2
      ...
      prim2(iby,j,k,l) = 0.5D0*dy2(k)/dcos2(k)*(DSIN(yF2(k))*by_face(j,k,l) + DSIN(yF2(k-1))*by_face(j,k-1,l))
    END DO
  END DO
END DO
  ...
  DO k = 1, ny_2
    DO j = 1, nx_2
      div_b = (xF2(j)**2*bx_face(j,k,l) - xF2(j-1)**2*bx_face(j-1,k,l))/(x2(j)**2*dx2(j)) + & 
              (DSIN(yF2(k))*by_face(j,k,l) - DSIN(yF2(k-1))*by_face(j,k-1,l))/(x2(j)*dcos2(k))!*DSIN(y2(k))*dy2(k))
