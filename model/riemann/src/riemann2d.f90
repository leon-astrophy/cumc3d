  ...
  DO k = 1, ny_2
    DO j = 1, nx_2
      IF(y2(k) <= 0.5d0) THEN
        ...
        prim2(ivel2_x,j,k,:) = 0.0D0
        call random_number(dummy)
        prim2(ivel2_y,j,k,:) = 0.01D0*dummy*(1.0d0 + COS(8.0d0*pi*(y2(k) - 0.5D0)/3.0d0))/2.0d0
        epsilon2(j,k,:) = prim2(itau2,j,k,:) / prim2(irho2,j,k,:) / (ggas2 - 1.0D0)
      ELSE
        prim2(irho2,j,k,:) = 2.0D0
        prim2(itau2,j,k,:) = 2.5D0 - 0.1D0 * prim2(irho2,j,k,:) * (y2(k) - 0.5D0)
...
elseif(test_model == 19) then
  ...
  DO k = 1, ny_2
    DO j = 1, nx_2
      ...
      epsilon2(j,k,:) = prim2(itau2,j,k,:) / prim2(irho2,j,k,:) / (ggas2 - 1.0D0)
      r = SQRT(xF2(j)**2 + yF2(k)**2) ! cell corner coordinate !
  ...
  DO k = 1, ny_2
    DO j = 1, nx_2
      bx_face(j,k,:) = (A_corner(j,k,:) - A_corner(j,k-1,:))/dy2(k)
      by_face(j,k,:) = -(A_corner(j,k,:) - A_corner(j-1,k,:))/dx2(j)
    ENDDO
  ENDDO
  ...
  DO k = 1, ny_2
    DO j = 1, nx_2
      prim2(ibx,j,k,:) = 0.5D0*(bx_face(j,k,:) + bx_face(j-1,k,:))
      prim2(iby,j,k,:) = 0.5D0*(by_face(j,k,:) + by_face(j,k-1,:))
    END DO
  END DO
