...
! Do for DM !
IF(RUNDM_flag) THEN
	DO k = ny_min_1 - 3, ny_part_1 + 2
      DO l = nz_min_1 - 3, nz_part_1 + 2
		   CALL ALPHASPLIT_X (alpha1_x(k,l,:), dummy2(:), k, l, 1)
	   END DO
   END DO
	DO j = nx_min_1 - 3, nx_part_1 + 2
      DO l = nz_min_1 - 3, nz_part_1 + 2
		   CALL ALPHASPLIT_Y (alpha1_y(j,l,:), dummy2(:), j, l, 1)
	   END DO
   END DO
	DO j = nx_min_1 - 3, nx_part_1 + 2
      DO k = ny_min_1 - 3, ny_part_1 + 2
		   CALL ALPHASPLIT_Z (alpha1_z(j,k,:), dummy2(:), j, k, 1)
	   END DO
   END DO
END IF

! Do for NM !
DO k = ny_min_2 - 3, ny_part_2 + 2
   DO l = nz_min_2 - 3, nz_part_2 + 2
		CALL ALPHASPLIT_X (dummy1(:), alpha2_x(k,l,:), k, l, 2)
	END DO
END DO
DO j = nx_min_2 - 3, nx_part_2 + 2
   DO l = nz_min_2 - 3, nz_part_2 + 2
		CALL ALPHASPLIT_Y (dummy1(:), alpha2_y(j,l,:), j, l, 2)
	END DO
END DO
DO j = nx_min_2 - 3, nx_part_2 + 2
   DO k = ny_min_2 - 3, ny_part_2 + 2
		CALL ALPHASPLIT_Z (dummy1(:), alpha2_z(j,k,:), j, k, 2)
	END DO
END DO
