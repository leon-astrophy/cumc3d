...
IF (RUNDM_flag) THEN
   cons1 (:,:,:,:) = u_old1 (:,:,:,:) + 0.391752226571890D0 * dt * l1 (:,:,:,:)
   CALL BOUNDARYU_DM
END IF

! NM sector !
cons2 (:,:,:,:) = u_old2 (:,:,:,:) + 0.391752226571890D0 * dt * l2 (:,:,:,:)
...
IF (RUNDM_flag) THEN
	cons1 (:,:,:,:) = 0.444370493651235D0 * u_old1 (:,:,:,:) + 0.555629506348765D0 * cons1 (:,:,:,:) + 0.368410593050371D0 * dt * l1 (:,:,:,:)
	u2_dm (:,:,:,:) = cons1 (:,:,:,:)
 	CALL BOUNDARYU_DM
END IF

! NM sector !
cons2 (:,:,:,:) = 0.444370493651235D0 * u_old2 (:,:,:,:) + 0.555629506348765D0 * cons2 (:,:,:,:) + 0.368410593050371D0 * dt * l2 (:,:,:,:)
u2_nm (:,:,:,:) = cons2 (:,:,:,:)
...
IF (RUNDM_flag) THEN   
	cons1 (:,:,:,:) = 0.620101851488403D0 * u_old1 (:,:,:,:) + 0.379898148511597D0 * cons1 (:,:,:,:) + 0.251891774271694D0 * dt * l1 (:,:,:,:)
	u3_dm (:,:,:,:) = cons1 (:,:,:,:)
	l3_dm (:,:,:,:) = l1 (:,:,:,:)	
 	CALL BOUNDARYU_DM
END IF
...
! NM sector ! 
cons2 (:,:,:,:) = 0.620101851488403D0 * u_old2 (:,:,:,:) + 0.379898148511597D0 * cons2 (:,:,:,:) + 0.251891774271694D0 * dt * l2 (:,:,:,:)
u3_nm (:,:,:,:) = cons2 (:,:,:,:)
l3_nm (:,:,:,:) = l2 (:,:,:,:)
...
IF (RUNDM_flag) THEN
	cons1 (:,:,:,:) = 0.178079954393132D0 * u_old1 (:,:,:,:)  + 0.821920045606868D0 * cons1 (:,:,:,:)  + 0.544974750228521D0 * dt * l1 (:,:,:,:) 
 	CALL BOUNDARYU_DM
END IF

! NM sector !
cons2 (:,:,:,:) = 0.178079954393132D0 * u_old2 (:,:,:,:) + 0.821920045606868D0 * cons2 (:,:,:,:) + 0.544974750228521D0 * dt * l2 (:,:,:,:)
...
IF (RUNDM_flag) THEN
	cons1 (:,:,:,:) = 0.517231671970585D0 * u2_dm (:,:,:,:) + 0.096059710526147D0 * u3_dm (:,:,:,:) + 0.386708617503269D0 * cons1 (:,:,:,:) &
		+ 0.063692468666290D0 * dt * l3_dm (:,:,:,:) + 0.226007483236906D0 * dt * l1 (:,:,:,:)
 	CALL BOUNDARYU_DM
END IF

! NM sector !
cons2 (:,:,:,:) = 0.517231671970585D0 * u2_nm (:,:,:,:) + 0.096059710526147D0 * u3_nm (:,:,:,:) + 0.386708617503269D0 * cons2 (:,:,:,:) &
	   			...
	   			IF(prim1(j,k,l,irho1) > prim1_a(irho1)) THEN
					...
					lambda3 = ABS(prim1(j,k,l,ivel1_z)) + cs1(j,k,l)
	     			...
	     			IF(coordinate_flag == 2) THEN
	 	      			dt_temp1 = MIN(dr1(j), x1(j)*sin1(j,k,l)*dy1, x1(j)*dz1)
	      			ELSEIF(coordinate_flag == 1) THEN
	    	   			dt_temp1 = MIN(dr1(j), x1(j)*dz1)
	      			ELSE
						dt_temp1 = dr1(j)
					END IF
		...
		DO l = -2, nz_2 + 3
	   		...
	   		IF(prim2(j,k,l,irho2) > prim2_a(irho2)) THEN
				...
				lambda3 = ABS(prim2(j,k,l,ivel2_z)) + cs2(j,k,l)
         		lambda = MAX(lambda1, lambda2, lambda3)
	     		IF(coordinate_flag == 2) THEN
	 	      		dt_temp2 = MIN(dr2(j), x2(j)*sin2(j,k,l)*dy2, x2(j)*dz2)
	      		ELSEIF(coordinate_flag == 1) THEN
	    	   		dt_temp2 = MIN(dr2(j), x2(j)*dz2)
	      		ELSE
					dt_temp2 = dr2(j)
				END IF
