...
IF (RUNDM_flag) THEN
	p1 (:,:,:) = kgas1 * prim1(:,:,:,irho1) ** ggas1
	dpdrho1 (:,:,:) = kgas1 * ggas1 * prim1(:,:,:,irho1) ** (ggas1 - 1.0D0)
	dpdeps1 (:,:,:) = 0.0D0
...
prim2(:,:,:,itau2) = prim2(:,:,:,irho2) * epsilon2(:,:,:) * (ggas2 - 1.0E0_DP) 
dpdrho2 (:,:,:) = epsilon2(:,:,:) * (ggas2 - 1.0E0_DP)
dpdeps2 (:,:,:) = prim2(:,:,:,irho2) * (ggas2 - 1.0E0_DP)
