...
def plot_contour(X1_in,X2_in,prim,filename,domain='xy',scale='log',stream=False,bh=False):
    ...
    # log scale plot #
    if(scale == 'log'):
        zmax = int(round(np.log10(np.max(prim))))
        zmin = int(round(np.log10(np.min(prim))))
        cp = ax.contour(X1_in, X2_in, prim, np.logspace(zmin, zmax, 100), locator=ticker.LogLocator(), cmap='RdGy',extend='both')
    ...
    # linear scale plot #
        ...
        cp = ax.contour(X1_in, X2_in, prim, 100, cmap='RdGy', extend='both')
        cbar = fig.colorbar(cp)
