...
IF(coordinate_flag == 2) THEN
	DO k = 0, ny_2 + 1
		dmu = DCOS(yF2(k-1)) - DCOS(yF2(k))
		dmubar = sin2f(k-1) - sin2f(k)
		hpy(k) = dy2(k)*(dmubar + dy2(k)*DCOS(yF2(k)))/(dy2(k)*(sin2f(k-1) + sin2f(k)) - 2.0d0*dmu)
		hmy(k) = -dy2(k)*(dmubar + dy2(k)*DCOS(yF2(k-1)))/(dy2(k)*(sin2f(k-1) + sin2f(k)) - 2.0d0*dmu)
	END DO
ELSE
	DO k = 0, ny_2 + 1
		hpy(k) = 3.0d0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Get matrix element for polar geometry i.e. dv/dx = DSIN(x)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
DO i = 1, 5
	DO j = 1, 5
		...
		DO k = 0, n_temp
			...
			*(left**(DBLE(n_temp - k))*DCOS(left + DBLE(k)*0.5d0*pi) - right**(DBLE(n_temp - k))*DCOS(right + DBLE(k)*0.5d0*pi))
		END DO
		matrix_out(i, j) = matrix_out(i, j)/(DCOS(left) - DCOS(right))
	END DO
END DO
...
WRITE(*,*) 'ppmx = ', REAL(time_end - time_start) / rate
#endif
...
#ifdef DEBUG
CALL system_clock(time_end)
WRITE(*,*) 'ppmy = ', REAL(time_end - time_start) / rate
#endif
...
#ifdef DEBUG
CALL system_clock(time_end)
WRITE(*,*) 'ppmz = ', REAL(time_end - time_start) / rate
#endif
