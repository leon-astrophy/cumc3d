...
IF(coordinate_flag == 2) THEN
	DO k = 0, ny_2 + 1
		...
		dmubar = DSIN(yF2(k-1)) - DSIN(yF2(k))
		hpy(k) = dy2(k)*(dmubar + dy2(k)*DCOS(yF2(k)))/(dy2(k)*(DSIN(yF2(k-1)) + DSIN(yF2(k))) - 2.0d0*dmu)
		hmy(k) = -dy2(k)*(dmubar + dy2(k)*DCOS(yF2(k-1)))/(dy2(k)*(DSIN(yF2(k-1)) + DSIN(yF2(k))) - 2.0d0*dmu)
	END DO
ELSE
	DO k = 0, ny_2 + 1
		hpy(k) = 3.0d0
