...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	compute_signalspeed = SQRT(0.5D0*(a2_mhd + b2_mhd + SQRT(MAX((a2_mhd + b2_mhd)**2 - 4.0D0*a2_mhd*bn_mhd**2, 0.0d0))))
	end function
	...
	DO k = ny_min_2 - 1, ny_part_2 + ky
		DO j = nx_min_2 - 1, nx_part_2 + kx
			...
			ELSE
				...
				! Get HLL state !
				u_hll(ibn) = compute_hll(primL2(ibn,j,k,l),primR2(ibn,j,k,l),fluxL2(ibn,j,k,l),fluxR2(ibn,j,k,l),sL,sR)
				...
				IF(sstar >= 0.0D0) THEN
					...
					! Star pressure !
					pLs = primL2(irho2,j,k,l)*(sL - primL2(ivn,j,k,l))*(sstar - primL2(ivn,j,k,l)) + pLt - uL2(ibn,j,k,l)**2 + u_hll(ibn)**2
				...
				ELSE
					...
					! Star pressure !
					pRs = primR2(irho2,j,k,l)*(sR - primR2(ivn,j,k,l))*(sstar - primR2(ivn,j,k,l)) + pRt - uR2(ibn,j,k,l)**2 + u_hll(ibn)**2
...
!$OMP sstarl, sstarr, bdotu, bdotus, bdotuss, sl, sr, sm, fac_1, fac_2, b_hll, & 
...
!$ACC b2l, b2r, plt, prt, pls, prs, sstarl, sstarr, bdotu, bdotus, bdotuss, sl, sr, sm, fac_1, fac_2, b_hll, & 
	...
	DO k = ny_min_2 - 1, ny_part_2 + ky
		DO j = nx_min_2 - 1, nx_part_2 + kx
			...
			ELSE
				...
				! b-Hll !
				b_hll = compute_hll(primL2(ibn,j,k,l),primR2(ibn,j,k,l),fluxL2(ibn,j,k,l),fluxR2(ibn,j,k,l),sL,sR)
				
				! Multi-state wave speed !
				sstarL = sM - ABS(b_hll)/SQRT(ustarL(irho2))
				sstarR = sM + ABS(b_hll)/SQRT(ustarR(irho2))
				...
				ELSE
					...
					! left states !
					fac_1 = (primL2(irho2,j,k,l)*(sL - primL2(ivn,j,k,l))*(sL - sM) - b_hll**2)
					ustarL(ibn) = b_hll
					ustarL(ibt1) = (primL2(ibt1,j,k,l)*(primL2(irho2,j,k,l)*(sL - primL2(ivn,j,k,l))**2 - b_hll*primL2(ibn,j,k,l)) &
												- primL2(irho2,j,k,l)*(sL - primL2(ivn,j,k,l))*(b_hll - primL2(ibn,j,k,l))*primL2(ivt1,j,k,l))/fac_1
					ustarL(ibt2) = (primL2(ibt2,j,k,l)*(primL2(irho2,j,k,l)*(sL - primL2(ivn,j,k,l))**2 - b_hll*primL2(ibn,j,k,l)) &
												- primL2(irho2,j,k,l)*(sL - primL2(ivn,j,k,l))*(b_hll - primL2(ibn,j,k,l))*primL2(ivt2,j,k,l))/fac_1

					! right states !
					fac_1 = (primR2(irho2,j,k,l)*(sR - primR2(ivn,j,k,l))*(sR - sM) - b_hll**2)
					ustarR(ibn) = b_hll
					ustarR(ibt1) = (primR2(ibt1,j,k,l)*(primR2(irho2,j,k,l)*(sR - primR2(ivn,j,k,l))**2 - b_hll*primR2(ibn,j,k,l)) &
											- primR2(irho2,j,k,l)*(sR - primR2(ivn,j,k,l))*(b_hll - primR2(ibn,j,k,l))*primR2(ivt1,j,k,l))/fac_1
					ustarR(ibt2) = (primR2(ibt2,j,k,l)*(primR2(irho2,j,k,l)*(sR - primR2(ivn,j,k,l))**2 - b_hll*primR2(ibn,j,k,l)) &
											- primR2(irho2,j,k,l)*(sR - primR2(ivn,j,k,l))*(b_hll - primR2(ibn,j,k,l))*primR2(ivt2,j,k,l))/fac_1
				...
				! Presure !
				pLs = primL2(irho2,j,k,l)*(sL - primL2(ivn,j,k,l))*(sM - primL2(ivn,j,k,l)) + pLt - uL2(ibn,j,k,l)**2 + b_hll**2
				pRs = primR2(irho2,j,k,l)*(sR - primR2(ivn,j,k,l))*(sM - primR2(ivn,j,k,l)) + pRt - uR2(ibn,j,k,l)**2 + b_hll**2
				...
				ustarL(itau2) = ustarL(itau2) + ((pLs*sM - pLt*primL2(ivn,j,k,l)) - (bdotus*b_hll - bdotu*primL2(ibn,j,k,l)))/(sL - sM)
				...
				ustarR(itau2) = ustarR(itau2) + ((pRs*sM - pRt*primR2(ivn,j,k,l)) - (bdotus*b_hll - bdotu*primR2(ibn,j,k,l)))/(sR - sM)
				...
				ELSE
					...
					! factor !
					fac_1 = 1.0d0 / (SQRT(ustarL(irho2)) + SQRT(ustarR(irho2)))
					fac_2 = SIGN(1.0d0,b_hll)

					! left states !
					psstarL(ivn) = sM 
					psstarL(ivt1) = fac_1*(SQRT(ustarL(irho2))*pstarL(ivt1) + SQRT(ustarR(irho2))*pstarR(ivt1) + (ustarR(ibt1) - ustarL(ibt1))*fac_2)
					psstarL(ivt2) = fac_1*(SQRT(ustarL(irho2))*pstarL(ivt2) + SQRT(ustarR(irho2))*pstarR(ivt2) + (ustarR(ibt2) - ustarL(ibt2))*fac_2)
					usstarL(ibt1) = fac_1*(SQRT(ustarL(irho2))*ustarR(ibt1) + SQRT(ustarR(irho2))*ustarL(ibt1) + SQRT(ustarL(irho2)*ustarR(irho2))*(pstarR(ivt1) - pstarL(ivt1))*fac_2)
					usstarL(ibt2) = fac_1*(SQRT(ustarL(irho2))*ustarR(ibt2) + SQRT(ustarR(irho2))*ustarL(ibt2) + SQRT(ustarL(irho2)*ustarR(irho2))*(pstarR(ivt2) - pstarL(ivt2))*fac_2)
					...
					usstarL(itau2) = ustarL(itau2) - SQRT(ustarL(irho2))*(bdotus - bdotuss)*fac_2
					...
					usstarR(itau2) = ustarR(itau2) + SQRT(ustarR(irho2))*(bdotus - bdotuss)*fac_2
