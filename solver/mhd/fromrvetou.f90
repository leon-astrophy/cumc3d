...
ELSEIF(coordinate_flag == 2) THEN
  ...
  DO l = nz_min_2, nz_part_2
    DO k = ny_min_2, ny_part_2
      DO j = nx_min_2, nx_part_2
        bcell(ibx,j,k,l) = 1.5D0*dx2(j)/dx2_cb(j)*(xF2(j)**2*prim2(ibx,j,k,l) + xF2(j-1)**2*prim2(ibx,j-1,k,l))
        bcell(iby,j,k,l) = 0.5D0*dy2(k)/dcos2(k)*(DSIN(yF2(k))*prim2(iby,j,k,l) + DSIN(yF2(k-1))*prim2(iby,j,k-1,l))
        bcell(ibz,j,k,l) = 0.5D0*(prim2(ibz,j,k,l) + prim2(ibz,j,k,l-1))
      END DO
    END DO
  END DO
