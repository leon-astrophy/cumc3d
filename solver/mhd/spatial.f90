...
! Geometric sources
!$OMP PARALLEL PRIVATE(bsquare, diff, factor)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Initialize !
  	...
  	DO k = ny_min_2, ny_part_2
      	...
      	factor = MAX(SIGN(1.0D0, diff), 0.0D0)
				...
				sc2(ivel2_y,j,k,l) = sc2(ivel2_y,j,k,l) + (factor*prim2(itau2,j,k,l) + factor*prim2(irho2,j,k,l)*prim2(ivel2_z,j,k,l)**2 & 
																								+ 0.5D0*bsquare - prim2(ibz,j,k,l)**2)*(1.5D0*dx2_sq(j)/dx2_cb(j))*(dsin2(k)/dcos2(k)) & !/DTAN(y2(k)) & !/x2(j)
				...
				sc2(ivel2_z,j,k,l) = sc2(ivel2_z,j,k,l) - (factor*prim2(irho2,j,k,l)*prim2(ivel2_x,j,k,l)*prim2(ivel2_z,j,k,l) & 
																							  ...
																							  - (factor*prim2(irho2,j,k,l)*prim2(ivel2_y,j,k,l)*prim2(ivel2_z,j,k,l) & 
																								- prim2(iby,j,k,l)*prim2(ibz,j,k,l))*(1.5D0*dx2_sq(j)/dx2_cb(j))*(dsin2(k)/dcos2(k)) !/DTAN(y2(k)) !/x2(j)
			END DO
		END DO
	END DO
	!$ACC END PARALLEL
	...
	ELSEIF(coordinate_flag == 2) THEN
		...
		DO l = nz_min_2, nz_part_2
			DO k = ny_min_2, ny_part_2
				DO j = nx_min_2, nx_part_2
					DO i = imin2, ibx - 1
						dflux_2 = (DSIN(yF2(k))*flux_2 (i,j,k,l) - DSIN(yF2(k-1))*flux_2 (i,j,k-1,l)) & 
															/ (x2bar(j)*dcos2(k))
						l2(i,j,k,l) = l2(i,j,k,l) - dflux_2
					END DO
				END DO
	...
	DO l = nz_min_2 - 1, nz_part_2
		DO k = ny_min_2 - 1, ny_part_2
			DO j = nx_min_2 - 1, nx_part_2
				...
				! dbx/dt !
				l2(ibx,j,k,l) = - ((efield_z(j,k,l) - efield_z(j,k-1,l))/(xF2(j)*dy2(k)) - (efield_y(j,k,l) - efield_y(j,k,l-1))/(dz2(l)))

				! dby/dt !
				l2(iby,j,k,l) = - ((efield_x(j,k,l) - efield_x(j,k,l-1))/(dz2(l)) - (efield_z(j,k,l) - efield_z(j-1,k,l))/(dx2(j)))

				! dbz/dt !
				l2(ibz,j,k,l) = - ((xF2(j)*efield_y(j,k,l) - xF2(j-1)*efield_y(j-1,k,l))/(dx2(j)) - (efield_x(j,k,l) - efield_x(j,k-1,l))/(dy2(k)))/x2(j)
	...
	DO l = nz_min_2  - 1, nz_part_2
		DO k = ny_min_2 - 1, ny_part_2
			DO j = nx_min_2 - 1, nx_part_2
				...
				! dbx/dt !
				l2(ibx,j,k,l) = - ((DSIN(yF2(k))*efield_z(j,k,l) - DSIN(yF2(k-1))*efield_z(j,k-1,l))/(dcos2(k)) - (efield_y(j,k,l) - efield_y(j,k,l-1))/(dz2(l))*DSIN(y2(k)))/(xF2(j))

				! dby/dt !
				l2(iby,j,k,l) = - ((efield_x(j,k,l) - efield_x(j,k,l-1))/(DSIN(yF2(k))*dz2(l)) - (xF2(j)*efield_z(j,k,l) - xF2(j-1)*efield_z(j-1,k,l))/(dx2(j)))/x2(j)
