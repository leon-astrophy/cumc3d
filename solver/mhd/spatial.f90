...
! Geometric sources
!$OMP PARALLEL PRIVATE(dpdx2, dpdy2, dpdz2, bsquare, diff, factor, rho_min2)
	...
	DO l = nz_min_2, nz_part_2
		DO k = ny_min_2, ny_part_2
			DO j = nx_min_2, nx_part_2
				...
				lint_x(j,k,l) = dbdt_r
				lint_x(j-1,k,l) = dbdt_l
				l2(ibx,j,k,l) = (xF2(j-1)*dbdt_l + xF2(j)*dbdt_r)/(xF2(j) + xF2(j-1))
	...
	DO l = nz_min_2, nz_part_2
		DO k = ny_min_2, ny_part_2
			DO j = nx_min_2, nx_part_2
				...
				! dbx/dt !
				dbdt_r = - ((SIN(yF2(k))*efield_z(j,k,l) - SIN(yF2(k-1))*efield_z(j,k-1,l))/(dcos2(k)) - (efield_y(j,k,l) - efield_y(j,k,l-1))/(dz2(l))*SIN(y2(k)))/(xF2(j))
				dbdt_l = - ((SIN(yF2(k))*efield_z(j-1,k,l) - SIN(yF2(k-1))*efield_z(j-1,k-1,l))/(dcos2(k)) - (efield_y(j-1,k,l) - efield_y(j-1,k,l-1))/(dz2(l))*SIN(y2(k)))/(xF2(j-1))
				lint_x(j,k,l) = dbdt_r
				lint_x(j-1,k,l) = dbdt_l
				l2(ibx,j,k,l) = 1.5D0*dx2(j)/dx2_cb(j)*(xF2(j-1)**2*dbdt_l + xF2(j)**2*dbdt_r)
				...
				lint_y(j,k,l) = dbdt_r
				lint_y(j,k-1,l) = dbdt_l
				l2(iby,j,k,l) = 0.5D0*dy2(k)/dcos2(k)*(SIN(yF2(k-1))*dbdt_l + SIN(yF2(k))*dbdt_r)
