...
DO l = nz_min_2, nz_part_2
	DO k = ny_min_2, ny_part_2
		DO j = nx_min_2, nx_part_2
			...
			a2_mhd = cs2(j,k,l)**2
			bx_mhd = DSQRT(bcell(ibx,j,k,l)**2/prim2(irho2,j,k,l))
			by_mhd = DSQRT(bcell(iby,j,k,l)**2/prim2(irho2,j,k,l))
			bz_mhd = DSQRT(bcell(ibz,j,k,l)**2/prim2(irho2,j,k,l))
			b2_mhd = bx_mhd**2 + by_mhd**2 + bz_mhd**2
			cfx_mhd = DSQRT(0.5D0*(a2_mhd + b2_mhd + DSQRT((a2_mhd + b2_mhd)**2 - 4.0D0*a2_mhd*bx_mhd**2)))
			cfy_mhd = DSQRT(0.5D0*(a2_mhd + b2_mhd + DSQRT((a2_mhd + b2_mhd)**2 - 4.0D0*a2_mhd*by_mhd**2)))
			cfz_mhd = DSQRT(0.5D0*(a2_mhd + b2_mhd + DSQRT((a2_mhd + b2_mhd)**2 - 4.0D0*a2_mhd*bz_mhd**2)))
			...
			ELSEIF(coordinate_flag == 2) THEN
				...
				END IF
				IF(n_dim > 2) THEN
					dt_temp2 = MIN(dt_temp2, x2(j)*DSIN(y2(k))*dz2(l))
				END IF
			END IF
